...
% titlestring=['\fontsize{20}Plot of E_z vs X & Y for 2D FDTD open boundary at time  =',num2str(i*dt)];
...
ylabel('\leftarrow Y');
zlabel('E_z \rightarrow');
titlestring=['\fontsize{20}Inverse 2d fdtd at time  =',num2str(i*dt)];
title(titlestring,'color','k');
%TE mode
axis([-L(1)/2 L(1)/2 -L(2)/2 L(2)/2 -4e5 4e5]);
...
%Create a VideoWriter object and set properties
myWriter = VideoWriter('2dfdtdinversewithpbc1');            %create an .avi file
% myWriter = VideoWriter('curve','MPEG-4');   %create an .mp4 file
myWriter.FrameRate = 20;
%Open the VideoWriter object, write the movie, and close the file
open(myWriter);
